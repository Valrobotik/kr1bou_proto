...
class Strategy:
    ...
    def debug_phase_goto(self):
        while not rospy.is_shutdown():
            self.go_to(2.55, 0.95, 3.49, MAX_SPEED, 1)
            self.wait_until_ready()
            self.go_to(.7, 0.37, 3.54, MEDIUM_SPEED, 1)
            self.wait_until_ready()
            break
    ...
    try:
        ...
        if not strategy_manager.reset_position_from_camera():
            ...
            if strategy_manager.team == TEAM_BLUE:
                strategy_manager.position = Pose2D(2.8, 1., 0.0) #TODO : change back
                strategy_manager.publisher_correct_odom.publish(Pose2D(2.8, 1., 0.0))
            else:
