#!/usr/bin/env python3
"""
Handles the on-pole camera. Used to have accurate position of our own robot, the adversary, and all other Aruco markers.
(objectives and map positions).
"""
...
def det_angle(img : np.ndarray, detector : cv2.aruco.ArucoDetector) :
    """
    Detects the angle of the camera with respect to the pole.
    """
...
if __name__ == "__main__" :
    ...
    rospy.loginfo("[START] Camera pole node has started.")
    ...
    cam_pole_pub = rospy.Publisher("camera_pole", Int8, queue_size=queue_size)
    ...
    while not rospy.is_shutdown() :
        ...
        if winner is not None :
            cam_pole_pub.publish(Int8(data=winner))
            rospy.loginfo("[CAMERA_POLE] winner : " + str(winner))
        rate.sleep()
